...
export async function subirFinal(formData: FormData) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      ...
      throw new Error(errorData.error || errorData.message || 'Datos inválidos o archivo no permitido.');
    } else if (response.status === 500) {
      // Error del servidor
      throw new Error(errorData.error || errorData.detalle || 'Error interno del servidor. Intenta nuevamente.');
    }

    // Para cualquier otro error, usar el mensaje del backend si existe
    throw new Error(errorData.error || errorData.message || `Error al subir el final (${response.status})`);
  }
