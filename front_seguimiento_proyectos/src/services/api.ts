...
export async function getStudentProfile() {
  try {
    ...
    if (!res.ok) {
      throw new Error(
        data?.message || "Error al obtener perfil del estudiante"
      );
    }
    return data;
  } catch (err) {
...
export async function createProyecto(titulo: string) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/proyecto/crear`,
      {
        method: "POST",
        credentials: "include",
        headers: {
...
export async function obtenerProyecto() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/estudiante/proyecto`,
      {
        credentials: "include",
      }
    );
...
export async function updateGroup(
  id: number,
  data: Partial<{ nombre: string; grado: string; docente_id?: number }>
) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/admin/grupos/${id}`,
    ...
    const response = await res.json();
    if (!res.ok)
      throw new Error(response.message || "Error al actualizar grupo");
    return response;
  } catch (err) {
    if (err instanceof Error) {
      throw new Error(err.message || "Error de conexión con el servidor");
