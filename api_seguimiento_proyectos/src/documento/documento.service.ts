...
export class DocumentoService {
  ...
  async getDocsByProyectoYGrupo(proyectoId: number, grupoId: number) {
  if (!proyectoId || isNaN(proyectoId) || proyectoId <= 0) {
    throw new BadRequestException('El ID del proyecto debe ser un número positivo');
  }

  if (!grupoId || isNaN(grupoId) || grupoId <= 0) {
    throw new BadRequestException('El ID del grupo debe ser un número positivo');
  }

  // Obtener documentos filtrando por proyecto y grupo de la actividad
  const documentos = await this.prisma.documento.findMany({
    where: {
      proyecto_id: proyectoId,
      actividad: {
  ...
  // Obtener finales filtrando por proyecto y grupo de la actividad
  const finales = await this.prisma.final.findMany({
    where: {
      proyecto_id: proyectoId,
      actividad: {
  ...
  const mappedDocs = documentos.map((doc) => ({
    id: doc.id,
    titulo: doc.titulo,
    version: doc.version,
    estado: doc.estado,
    ...
    tipo: "documento",
    actividad: doc.actividad ? {  // ⬅️ AGREGAR ESTO
      nombre: doc.actividad.nombre,
    } : null,
    proyecto: {
      id: doc.proyecto.id,
      titulo: doc.proyecto.titulo,
  ...
  // Mapear finales al formato requerido
  const mappedFinales = finales.map((final) => ({
    id: final.id,
    titulo: final.titulo,
    carrera: final.carrera,
    año: final.año,
    ...
    archivo: final.archivo,
    fase: final.fase,
    actividad_id: final.actividad_id,
    tipo: "final",
    tags: final.tags,
    actividad: final.actividad ? {  // ⬅️ AGREGAR ESTO
      nombre: final.actividad.nombre,
    } : null,
    proyecto: {
      id: final.proyecto.id,
      titulo: final.proyecto.titulo,
  ...
  // Unificar documentos + finales
  return [...mappedDocs, ...mappedFinales];
}
