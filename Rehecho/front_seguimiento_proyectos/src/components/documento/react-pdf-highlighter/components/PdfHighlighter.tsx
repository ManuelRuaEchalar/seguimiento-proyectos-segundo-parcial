...
> {
  ...
  // Dentro de init(), después de attachRef (línea ~210):
async init() {
  this.isReady = false;
  console.log("=== DEBUG: init() started");
  
  // Limpiar raíces existentes antes de reinicializar
  this.cleanupHighlightRoots();
  ...
  // ✅ AGREGAR FLAG PARA CONTROLAR FALLBACKS
  let fallbacksCleared = false;
  const timeoutIds: NodeJS.Timeout[] = [];

  const checkPagesReady = () => {
    if (this.isReady) { // ✅ Si ya está listo, no hacer nada
      return true;
    }
  ...
  // ✅ GUARDAR IDs DE TIMEOUTS PARA PODER CANCELARLOS
  ...
  // ✅ REMOVER EL LISTENER pagerendered QUE CAUSA MÚLTIPLES LLAMADAS
  // En lugar de esto, confiar solo en 'pagesinit' que ya está en attachRef
}
...
private cleanupHighlightRoots() {
  Object.values(this.highlightRoots).forEach(({ reactRoot }) => {
    try {
      // ✅ Usar setTimeout para desmontar después del render actual
      setTimeout(() => {
        reactRoot.unmount();
      }, 0);
    } catch (e) {
      console.warn('Error unmounting highlight root:', e);
    }
  });
  this.highlightRoots = {};
}
  ...
  onDocumentReady = () => {
  if (this.isReady) {
    console.log("=== DEBUG: onDocumentReady ya fue ejecutado, saltando...");
    return;
  }
  this.isReady = true;
  ...
  // ✅ FORZAR RENDERIZADO INICIAL después de que todo esté listo
  setTimeout(() => {
    this.renderHighlightLayers();
  }, 100);
};
  ...
  private renderHighlightLayers() {
    ...
    for (let pageNumber = 1; pageNumber <= pdfDocument.numPages; pageNumber++) {
      ...
      // Si ya existe una raíz Y el contenedor sigue conectado al DOM
      if (existingRoot?.container.isConnected) {
        // Solo renderizar en la raíz existente
        this.renderHighlightLayer(existingRoot.reactRoot, pageNumber);
      } else {
        // Si existía una raíz pero el contenedor fue desconectado, limpiarla
        if (existingRoot) {
          try {
            existingRoot.reactRoot.unmount();
          } catch (e) {
            console.warn(`Error unmounting root for page ${pageNumber}:`, e);
          }
          delete this.highlightRoots[pageNumber];
        }
        
        // Crear nueva raíz solo si no existe
        const highlightLayer = this.findOrCreateHighlightLayer(pageNumber);
        if (highlightLayer) {
          // Verificar que no haya una raíz React ya creada en este elemento
