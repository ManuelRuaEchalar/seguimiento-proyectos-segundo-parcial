...
async upload(
  ...
  try {
    ...
    if (!titulo || !carrera || !anio || !fase || !proyecto_id) { // 👈 CAMBIAR
      console.error('❌ Faltan datos:', { titulo, carrera, anio, fase, proyecto_id }); // 👈 CAMBIAR
      return res.status(400).json({ 
        success: false, 
        error: 'Faltan datos requeridos',
        detalles: { titulo: !!titulo, carrera: !!carrera, anio: !!anio, fase: !!fase, proyecto_id: !!proyecto_id } // 👈 CAMBIAR
      });
    }
    ...
    const nuevoFinal = await this.finalService.crearFinal({
      titulo,
      carrera,
    ...
    console.log('✅ Final creado:', nuevoFinal);
